...
<div class="site-page-shell">
    <div class="container">
        <section class="site-page-hero text-center text-lg-start">
            ...
            <h1 class="site-page-hero__title" lang="{{ $locale }}" dir="{{ $dir }}">{{ __('gallery.page_heading') }}
            </h1>
        ...
        <section class="site-panel soft-panel">
            <div class="site-panel-body">
                <div class="row g-4 site-grid-stretch">
                    ...
                    <div class="col-md-6 col-lg-4">
                        <div class="site-content-card">
                            <div class="site-content-card__media">
                                @if ($img->isVideo())
                                <video controls playsinline preload="metadata" src="{{ $imgUrl }}"
                                    title="{{ $img->title ?? __('gallery.gallery_image') }}"></video>
                                @else
                                <img src="{{ $imgUrl }}" alt="{{ $img->title ?? __('gallery.gallery_image') }}"
                                    loading="lazy">
                                @endif
                            </div>
                            <div class="site-content-card__body text-center">
                                <h5 class="site-section-title mb-2" style="font-size: 1.2rem;" lang="{{ $locale }}"
                                    dir="{{ $dir }}">{{ $img->title ?? __('gallery.gallery_image') }}</h5>
                                <p class="site-section-copy mb-1">{{ $img->created_at?->format('d M, Y') }}</p>
                                <p class="site-section-copy mb-0">{{ $img->user->name ?? __('gallery.unknown_user') }}
                                </p>
                            </div>
                        </div>
                    </div>
                    @empty
                    <div class="col-12">
                        <div class="site-empty" lang="{{ $locale }}" dir="{{ $dir }}">{{ __('gallery.no_images') }}
                        </div>
                    </div>
                    @endforelse
                </div>
